...
export default class ColorPicker {
  constructor(main, callback) {
    ...
    this.colorsArea = main.wrapper.querySelector('.ptro-color-widget-wrapper .colors');
    if(main.params.palette){

      main.params.palette.forEach((v, i) => { 
        main.wrapper.querySelector('.ptro-color-widget-wrapper .picker').style.visibility='hidden';
        var sonDiv = document.createElement("div");
        sonDiv.className="paletteColor";
        var tt = rgbToHex(v[0],v[1],v[2]);
        sonDiv.style.backgroundColor=rgbToHex(v[0],v[1],v[2]);
        var that=this;
        //sonDiv.οnclick=
        sonDiv.addEventListener('click',function(){
          that.setActiveColor(tt);
      })
        this.colorsArea.appendChild(sonDiv);
      })
    }
    ...
    return '' +
      '<div class="ptro-color-widget-wrapper ptro-common-widget-wrapper ptro-v-middle" hidden>' +
        '<div class="ptro-pallet ptro-color-main ptro-v-middle-in">' +
          '<div class="colors" style="position: absolute;width: 200px;align-items: left;"></div>'+
          '<div class="picker"><canvas></canvas>' +
