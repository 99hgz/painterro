...
export default class PrimitiveTool {
  ...
  _fillRect(colorLayerData,fillStyle,x,y,w,h){
    x=Math.floor(x)
    y=Math.floor(y)
    let color = HexToRGB(fillStyle);
    let r=color.r;
    let g=color.g;
    let b=color.b;
    //console.log(r,g,b,x,y,w,h);
    for(let j = y; j < y + h;j ++){
      let pixelPos = (j * this.canvasWidth + x) * 4;
      for(let i = x; i < x + w; i ++){
  ...
  drawBrushPath() {
    ...
    //let colorLayerData = this.ctx.getImageData(0, 0, this.canvasWidth, this.canvasHeight);
    ...
    for (let i = 1; i <= (isEraser && bgIsTransparent ? 2 : 1); i += 1) {
      ...
      var _width = Math.floor(this.lineWidth / 2)
      if (smPoints.length === 1) {
        ...
        //  0, 2 * Math.PI);
        this.ctx.fillRect(Math.floor(this.points[0].x)-_width,Math.floor(this.points[0].y-_width),_width*2+1,_width*2+1);
        this.ctx.fill();
        this.ctx.closePath();
      } else {
        this.ctx.beginPath();
        ...
        smPoints.slice(1).forEach((p) => {
          //console.log('draw',p.x,p.y);
          //this.ctx.lineTo(p.x, p.y);
          //this._fillRect(colorLayerData,this.ctx.fillStyle,p.x-_width, p.y-_width, _width*2+1, _width*2+1)
          this.ctx.fillRect(Math.floor(p.x)-_width, Math.floor(p.y)-_width, _width*2+1, _width*2+1)
          last = p;
        });
        if (last) {
